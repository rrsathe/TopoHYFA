...
if "Symbol" not in adata.var:
    gene_idx_mask = adata.var.index.isin(target_genes_symbols)
...
for t in ttissues:
    ...
    if len(np.unique(patients_source_val)) >= 25:
        ...
        with torch.no_grad():
            ...
            out, node_features = forward(d, model, device, preprocess_fn=None)
            y_val_pred = out["px_rate"].cpu().numpy()
            y_test_ = d.x_target.cpu().numpy()

...
class PlainDecoder(torch.nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if (
            getattr(self, "lambda_reg", 0.0) > 0
            and getattr(self, "adjacency_matrix", None) is not None
        ):
            out["reg_loss"] = graph_laplacian_regularization(
                self.px_rate_decoder[0].weight, self.adjacency_matrix
            )
...
class PoissonDecoder(torch.nn.Module):
    ...
    def forward(self, x, log_library=None, **kwargs):
        ...
        out = {
            "px_scale": px_scale,
            "px_rate": px_rate,
            "library_mean": log_library_mean,
            "library_var": log_library_var,
            "library": log_library,
        ...
        if (
            ...
            out["reg_loss"] = graph_laplacian_regularization(
                self.px_scale_decoder[0].weight, self.adjacency_matrix
            )
            out["lambda_reg"] = self.lambda_reg
        return out
...
class ZeroInflatedNegativeBinomialDecoder(torch.nn.Module):
    ...
    def forward(self, x, log_library, n_cells=1.0, use_library_mean=False, **kwargs):
        """
        Reference: SCVI-tools.
        See: https://github.com/scverse/scvi-tools/blob/2472b76572672fb9c98755522fa3c7326a946444/scvi/nn/_base_components.py#L374
        :param x:

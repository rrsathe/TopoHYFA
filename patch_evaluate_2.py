...
for cell in nb.cells:
    if cell.cell_type == 'code':
        if "adata = GTEx_v8_normalised_adata(file=GTEX_FILE)" in cell.source and "target_genes_df = pd.read_csv" not in cell.source:
            cell.source = cell.source + "\\n" + filter_code_to_add
            print("Added filtering to adata cell.")
        if "test_mask = np.isin(donors, test_donors)" in cell.source and "gtex_test.txt" not in cell.source:
            # Fix split logic
